...
#' load samples with proteomics data
proteomics_samples <- read_tsv(file = "./output/files/proteomics_samples.txt") %>%
  mutate(batch = fct_rev(fct_infreq(batch)))



#' ## number of samples in each experimental batch
samples_barplot <- ggplot(data = proteomics_samples) +
...
ggsave(filename="protein_samples.png", plot = samples_barplot, path = "./output/plots/pre_process/", width = 10, height = 5)
unlink("protein_samples.png")



#' load proteomics data
protein <- fread(input = "./output/files/proteomics.txt.gz") %>%
  as_tibble() %>%
  gather(key = "sample", value = "log2FC", -gene) %>%
  inner_join(proteomics_samples %>% mutate_if(is.factor, as.character), by = "sample")



#' ## number of proteins by experimental batch
proteins_barplot <- protein %>%
  group_by(batch, gene) %>%
  filter( !(sum(is.na(log2FC)) == n()) ) %>%
  group_by(batch) %>%
  summarise(Proteins = length(unique(gene))) %>%
  ungroup() %>%
  gather(key = "feature", value = "n", -batch) %>%
  mutate(batch = fct_reorder(.f = batch, .x = n, .fun = median, .desc = FALSE)) %>%
  ggplot() +
...
ggsave(filename="number_proteins.png", plot = proteins_barplot, path = "./output/plots/pre_process/", width = 10, height = 5)
unlink("number_proteins.png")



#' ## log2FC distribution by sample and experimental batch
protein_distribution_1 <- ggplot(data = protein, mapping = aes(x = sample, y = log2FC, fill = cancer)) +
  theme_classic() +
...
protein_distribution_1

ggsave(filename="protein_distribution_1.png", plot = protein_distribution_1, path = "./output/plots/pre_process/", width = 30, height = 20)
unlink("protein_distribution_1.png")



#' ## log2FC distribution by sample and experimental batch (without outliers)
protein_distribution_2 <- ggplot(data = protein, mapping = aes(x = sample, y = log2FC, fill = cancer)) +
  theme_classic() +
...
#+ fig.width=30, fig.height=20
protein_distribution_2

ggsave(filename="protein_distribution_2.png", plot = protein_distribution_2, path = "./output/plots/pre_process/", width = 30, height = 20)
unlink("protein_distribution_2.png")



#' ## summary statistics by experimental batch
summary_stats <- protein %>%
  group_by(batch, gene) %>%
  filter( !(sum(is.na(log2FC)) == n()) ) %>%
  ungroup() %>%
...
ggsave(filename="proteomics_summary_stats.png", plot = summary_stats, path = "./output/plots/pre_process/", width = 15, height = 10)
unlink("proteomics_summary_stats.png")



#' ## number of samples whose distribution median exceeds 1/2/3 z-scores
n_outlier_samples1 <- protein %>%
  group_by(batch, gene) %>%
  filter( !(sum(is.na(log2FC)) == n()) ) %>%
  ungroup() %>%
  select(batch, sample, log2FC) %>%
...
ggsave(filename="protein_n_outlier_samples1.png", plot = n_outlier_samples_plot1, path = "./output/plots/pre_process/", width = 8, height = 6)
unlink("protein_n_outlier_samples1.png")



#' ## number of samples whose distribution median exceeds log2(2)/(3)/(4)
n_outlier_samples2 <- protein %>%
  group_by(batch, gene) %>%
  filter( !(sum(is.na(log2FC)) == n()) ) %>%
  ungroup() %>%
  select(batch, sample, log2FC) %>%
...
ggsave(filename="protein_n_outlier_samples2.png", plot = n_outlier_samples_plot2, path = "./output/plots/pre_process/", width = 8, height = 6)
unlink("protein_n_outlier_samples2.png")
...
#' ## log2FC distribution by sample and experimental batch (without outliers)
protein_distribution_3 <- ggplot(data = protein, mapping = aes(x = sample, y = log2FC, fill = cancer)) +
  theme_classic() +
  geom_boxplot(outlier.shape = NA, lwd = 0.1) +
  facet_wrap(~ batch, ncol = 3, scales = "free") +
  theme(
...
#+ fig.width=30, fig.height=20
protein_distribution_3

ggsave(filename="protein_distribution_3.png", plot = protein_distribution_3, path = "./output/plots/pre_process/", width = 30, height = 20)
unlink("protein_distribution_3.png")
...
write.table(protein, gz1, sep="\t", quote=F, row.names=F)
close(gz1)
