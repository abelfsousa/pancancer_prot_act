# regress-out protein and batch from phosphosites
...
args <- c("./output/files/phosphoproteomicsQ.txt.gz", "./output/files", "phosphoproteomicsQ_ProtBatchReg", "allSamp", "withZtransf")
...
if(args[4] == "allSamp"){
  reg <- pho_prot %>%
    select(psite, psites, prot, sample, psite_fc, prot_fc, batch) %>%
