...
kinase_pwm <- getKinasePWMs(db_tm, prot_seqs, k=7, rm_kmer=T, n_kmers=10, rm_kinase=T)
...
gz1 <- gzfile("./output/files/BKphospho_kinase_activity_ztestW2.txt.gz", "w")
...
gz1 <- gzfile("./output/files/BKphospho_kinase_activity_ztestNW2.txt.gz", "w")
write.table(inference, gz1, sep="\t", quote=F, row.names=F)
close(gz1)
