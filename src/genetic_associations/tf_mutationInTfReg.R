# Estimate the association between TF activity and the mutational state of the TF itself
...
associations <- TfA_mut(mutMat = mut, tfA = tf, covars = batch, multi_core = F, in_tf = T)

#plotTF(mut, tf, "TP53", "TP53")

# write associations
write_tsv(associations, "./output/files/tf_mutStatus_inTF.txt.gz")
