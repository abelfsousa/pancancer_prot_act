#' ---
#' title: "Associations between TF activities and mutations in all genes"
#' author: "Abel Sousa"
#' ---
...
volcano <- associations %>%
  ...
  mutate(l = pmap_chr(.l = ., .f = ~ if(..7 > 4){str_c(..4, ..3, sep = "--")}else{NA})) %>%
...
ggsave(filename = "tf_association_pairs_volcano_plot.png", plot = volcano, path = "./output/plots/genetic_associations/", width = 6, height = 6)
unlink("tf_association_pairs_volcano_plot.png")
...
volcano <- associations %>%
  filter(gene != tf) %>%
  mutate(l = pmap_chr(.l = ., .f = ~ helper(..7, ..8, ..3, ..4, W = 400, P = 0.0001))) %>%
  mutate(p.adjust = -log10(p.adjust)) %>%
...
ggsave(filename = "tf_association_pairs_volcano_plot2.png", plot = volcano, path = "./output/plots/genetic_associations/", width = 6, height = 6)
unlink("tf_association_pairs_volcano_plot2.png")
...
volcano <- associations %>%
  filter(gene != tf) %>%
  mutate(pair = pmap_chr(.l = ., .f = ~ if(is.na(..8)){NA}else if(..7 < 0.0001 & ..8 >= 800){str_c(..4, ..3, sep = "--")}else{NA})) %>%
  ...
  geom_text_repel(size = 3.5, segment.size = 0.3, color = "black", point.padding = 0.5, box.padding = 0.5, nudge_x = -1) +
  ...
  labs(x = expression("Mutation" ~ beta ~ "(effect size)"), y = "Adjusted P-value (-log10)", color = "String\nweight", title = "Transcription factors") +
  guides(color = guide_legend(override.aes = list(alpha=1, size = 2), nrow = 2))

#+ fig.width=4, fig.height=6
volcano

ggsave(filename = "tf_association_pairs_volcano_plot3.png", plot = volcano, path = "./output/plots/genetic_associations/", width = 4, height = 6, dpi = 900)
ggsave(filename = "tf_association_pairs_volcano_plot3.pdf", plot = volcano, path = "./output/plots/genetic_associations/", width = 4, height = 6)
unlink("tf_association_pairs_volcano_plot3.png")
unlink("tf_association_pairs_volcano_plot3.pdf")
...
volcano <- associations %>%
  ...
  ggplot(mapping = aes(x = estimate, y = p.adjust, color = string, label = pair)) +
  geom_point(mapping = aes(alpha = string), size = 1) +
  geom_text_repel(size = 3.5, segment.size = 0.3, color = "black", point.padding = 0.5, box.padding = 0.5, seed = 123, direction = "y", hjust = -0.3) +
  geom_vline(xintercept = 0, color = "black", linetype = "longdash", size = 0.1) +
  geom_hline(yintercept = -log10(0.05), color = "black", linetype = "longdash", size = 0.1) +
  geom_hline(yintercept = -log10(0.01), color = "black", linetype = "longdash", size = 0.1) +
  theme_classic() +
  ...
  scale_color_manual(values = c("#636363", "#6baed6", "#2171b5", "#08306b")) +
  #scale_x_continuous(limits = c(-3,3), breaks = seq(-3,3,1)) +
  labs(x = expression("Mutation" ~ beta ~ "(effect size)"), y = "Adjusted P-value (-log10)", color = "String network\nedge weight", title = "Transcription factors") +
  guides(color = guide_legend(override.aes = list(alpha=1, size = 2), nrow = 2))

#+ fig.width=5, fig.height=5
volcano

ggsave(filename = "tf_association_pairs_volcano_plot4.png", plot = volcano, path = "./output/plots/genetic_associations/", width = 5, height = 5, dpi = 900)
ggsave(filename = "tf_association_pairs_volcano_plot4.pdf", plot = volcano, path = "./output/plots/genetic_associations/", width = 5, height = 5)
unlink("tf_association_pairs_volcano_plot4.png")
unlink("tf_association_pairs_volcano_plot4.pdf")
...
volcano <- associations %>%
  ...
  geom_text_repel(size = 4, segment.size = 0.3, color = "black", point.padding = 0.5, box.padding = 0.5, seed = 123, direction = "y", hjust = -0.3) +
  geom_vline(xintercept = 0, color = "black", linetype = "longdash", size = 0.1) +
  geom_hline(yintercept = -log10(0.05), color = "black", linetype = "longdash", size = 0.1) +
  geom_hline(yintercept = -log10(0.01), color = "black", linetype = "longdash", size = 0.1) +
  theme_classic() +
...
ggsave(filename = "tf_association_pairs_volcano_plot5.png", plot = volcano, path = "./output/plots/genetic_associations/", width = 5, height = 5, dpi = 900)
ggsave(filename = "tf_association_pairs_volcano_plot5.pdf", plot = volcano, path = "./output/plots/genetic_associations/", width = 5, height = 5)


#' plot TFs and mutated genes involved in the associations
...
plots <- plot_grid(plot_TFs, plot_genes)


#+ fig.width=8, fig.height=8
plots

ggsave(filename = "tf_association_pairs_most_frequent_mut_genes_tfs.png", plot = plots, path = "./output/plots/genetic_associations/", width = 8, height = 8)
unlink("tf_association_pairs_most_frequent_mut_genes_tfs.png")


#' compare distribution of edge weights between significant and non-significant associations
p_values <- associations %>%
  filter(!is.na(weight)) %>%
  mutate(signf = if_else(p.adjust < 0.01, "Adj.P < 1%", "Adj.P > 1%")) %>%
  select(signf, weight) %>%
  nest() %>%
...
plot <- associations %>%
  filter(!is.na(weight)) %>%
  mutate(signf = if_else(p.adjust < 0.01, "Adj.P < 1%", "Adj.P > 1%")) %>%
  mutate(signf = fct_relevel(signf, "Adj.P > 1%")) %>%
...
ggsave(filename = "tf_association_pairs_weights_distribution.png", plot = plot, path = "./output/plots/genetic_associations/", width = 5, height = 1.5)
ggsave(filename = "tf_association_pairs_weights_distribution.pdf", plot = plot, path = "./output/plots/genetic_associations/", width = 5, height = 1.5)
unlink("tf_association_pairs_weights_distribution.png")
unlink("tf_association_pairs_weights_distribution.pdf")


plot <- associations %>%
  mutate(signf = if_else(p.adjust < 0.01, "2", "1")) %>%
  ggplot(mapping = aes(x = signf, y = weight, fill = signf)) +
  geom_violin() +
  geom_boxplot(outlier.shape = NA, width = 0.1) +
  theme_classic() +
  stat_compare_means(label.y = 600, label.x = 2.4, size = 3) +
  scale_x_discrete(labels = c("P > 1%", "P < 1%"), name = "Adjusted P-value") +
  scale_y_continuous(name = "String network edge weight") +
...
ggsave(filename = "tf_association_pairs_weights_distribution2.png", plot = plot, path = "./output/plots/genetic_associations/", width = 4, height = 2)
unlink("tf_association_pairs_weights_distribution2.png")
...
ggsave(filename = "tf_association_pairs_weights_distribution3.png", plot = plot, path = "./output/plots/genetic_associations/", width = 6, height = 3)
unlink("tf_association_pairs_weights_distribution3.png")
...
ggsave(filename = "tf_association_pairs_weights_distribution4.png", plot = plot, path = "./output/plots/genetic_associations/", width = 5, height = 1.5)
ggsave(filename = "tf_association_pairs_weights_distribution4.pdf", plot = plot, path = "./output/plots/genetic_associations/", width = 5, height = 1.5)
unlink("tf_association_pairs_weights_distribution4.png")
unlink("tf_association_pairs_weights_distribution4.pdf")
...
examples <- associations %>%
  mutate(pair = pmap_chr(.l = ., .f = ~ if(is.na(..8)){NA}else if(..7 < 0.0001 & ..8 >= 800){str_c(..4, ..3, sep = "--")}else{NA})) %>%
  filter(!is.na(pair)) %>%
  filter(pair %in% c("CTNNB1--TCF7L2", "TP53--E2F4", "RB1--E2F3", "TP53--TFDP1")) %>%
  mutate(pair = fct_reorder(pair, p.adjust, median)) %>%
  #mutate(pair = fct_recode(pair, `CTNNB1--\nTCF7L2` = "CTNNB1--TCF7L2", `TP53--\nTFDP1` = "TP53--TFDP1", `TP53--\nE2F4` = "TP53--E2F4", `RB1--\nE2F3` = "RB1--E2F3")) %>%
  inner_join(tf, by = "tf") %>%
...
dummy <- examples %>%
  ...
  ungroup() %>%
  mutate(activity = map(.x = activity, .f = helper, value = 1.5)) %>%
  unnest()
...
plot <- examples %>%
  ...
  geom_text(data = N, mapping = aes(x = pair, y = -15, label = n, group = mutated), size = 4, position = position_dodge(width = 0.8)) +
  ...
  scale_x_discrete(labels = c("CTNNB1--TCF7L2" = "CTNNB1\nTCF7L2", "TP53--E2F4" = "TP53\nE2F4", "RB1--E2F3" = "RB1\nE2F3", "TP53--TFDP1" = "TP53\nTFDP1")) +
  theme_classic() +
  theme(
...
ggsave(filename = "tf_association_pairs_examples1.png", plot = plot, path = "./output/plots/genetic_associations/", width = 5, height = 5)
ggsave(filename = "tf_association_pairs_examples1.pdf", plot = plot, path = "./output/plots/genetic_associations/", width = 5, height = 5)
