#' ---
#' title: "Immune infiltration (using relative cibersort estimates)"
#' author: "Abel Sousa"
#' ---
...
#' load immune cells fractions (relative values)\
#' estimated by Danish using CIBERSORT
ciber_relative <- read_csv(file = "./data/cibersort/CIBERSORT.CPTAC.relative.csv") %>%
  ...
  pivot_longer(-sample, names_to = "cell", values_to = "fraction")
...
cor_mat <- ciber_relative %>%
  inner_join(umap, by = "sample") %>%
  group_by(cell, umap) %>%
  nest() %>%
  ungroup() %>%
  mutate(cor = map(.x=data, .f = ~ broom::tidy(cor.test(.x$fraction, .x$value)) %>% select(estimate, p.value))) %>%
  select(-data) %>%
  unnest()
...
pdf(file = "./output/plots/immune_infiltration/umap_immune_relative_cor.pdf", width = 4, height = 4)
heatmap
dev.off()
...
umap_plot <- umap %>%
  ...
  inner_join(ciber_relative, by = "sample") %>%
  filter(cell %in% sel_cells) %>%
  ggplot(mapping = aes(x = UMAP1, y = UMAP2, color = fraction)) +
  geom_point(size = 0.5) +
  facet_wrap(~ cell) +
  theme_minimal() +
...
ggsave(filename = "umap_projection_immune_relative_cor.pdf", plot = umap_plot, path = "./output/plots/immune_infiltration/", width = 8, height = 4)
unlink("umap_projection_immune_relative_cor.pdf")
...
pdf(file = "./output/plots/immune_infiltration/pcs_immune_relative_cor.pdf", width = 6, height = 4)
heatmap
dev.off()
...
cor_mat <- ciber_relative %>%
  ...
  nest() %>%
  ungroup() %>%
  mutate(cor = map(.x=data, .f = ~ broom::tidy(cor.test(.x$fraction, .x$activity)) %>% select(estimate, p.value))) %>%
  select(-data) %>%
  unnest()
...
pdf(file = "./output/plots/immune_infiltration/kin_act_immune_relative_cor_all_kinases.pdf", width = 8, height = 12)
heatmap
dev.off()
...
pdf(file = "./output/plots/immune_infiltration/kin_act_immune_relative_cor_most_variable_kinases.pdf", width = 8, height = 12)
heatmap
dev.off()
