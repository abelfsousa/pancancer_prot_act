#' ---
#' title: "Protein regressed-out phosphorylation data to protein/RNA correlation"
#' author: "abelsousa"
#' ---
...
phospho <- read_tsv(file = "./output/files/phosphoproteomicsQ_Protreg.txt.gz") %>%
...
ggsave(filename="PhosQ_protregout_ProtRNA_corr_batch.png", plot = plot, path = "./output/plots/phospho_ProtRNA_correlation/", width=8, height=5)
...
plot <- plot_data %>%
  ...
  geom_text(
    data = plot_data %>%
      ...
      tally() %>%
      ungroup() %>%
      mutate(x = c(0.75, seq(0.5, 1, by = 0.5))) %>%
      mutate(y = -0.5),
    mapping = aes(x = x, y = y, color = genes, label=n),
    show.legend = F, size=3, nudge_x = 0.2) +
  facet_wrap(~ prot_data,
...
ggsave(filename="PhosQ_protregout_ProtRNA_corr.png", plot = plot, path = "./output/plots/phospho_ProtRNA_correlation/", width=5, height=4)
