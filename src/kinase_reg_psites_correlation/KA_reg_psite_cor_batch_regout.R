...
#ka_protreg <- read_tsv(file = "./output/files/CPTAC_KA_phosphoQ_ProtBatchRegout_allSamp_withZtransf_notKinSites.txt.gz") %>%
ka_protreg <- read_tsv(file = "./output/files/CPTAC_KA_phosphoQ_ProtBatchRegout_allSamp_withZtransf.txt.gz") %>%
  filter(source_type == "DB_text-mining") %>%
  filter(n >= k_subN) %>%
  select(-n, -source_type)

ka_kinPsite_protreg <- read_tsv(file = "./output/files/CPTAC_KA_kin_psites_phosphoQ_ProtBatchRegout_allSamp_withZtransf.txt.gz") %>%
  filter(n >= k_psit) %>%
  select(-n) %>%
  rename(kinase=gene)
...
phospho <- read_tsv(file = "./output/files/phosphoproteomicsQ_ProtBatchReg_allSamp_withZtransf.txt.gz") %>%
...
KA_RegPsite_cor_distribution <- all_cor %>%
  ...
  ggplot() +
  geom_boxplot(mapping = aes(x = PSP_reg_status, y = pearson_r, fill = PSP_reg_status), notch = T, outlier.shape = NA, color = "black", alpha = 0.8) +
  geom_jitter(mapping = aes(x = PSP_reg_status, y = pearson_r, alpha = PSP_reg_status), width = 0.1, color = "black") +
  scale_fill_manual(values = c("#a6611a", "#018571")) +
  scale_alpha_manual(values = c(0.05,0.2), guide = F) +
  stat_compare_means(mapping = aes(x = PSP_reg_status, y = pearson_r), method = "t.test", label.y = 1.1, size = 5, label.sep = ",\n") +
  theme_classic() +
  theme(
    ...
    legend.justification = c(0.5,1.7),
    legend.margin = margin(c(-0.2,0,0,0), unit="cm"),
    legend.text = element_text(colour = "black", size = 16)) +
  labs(x = "PsP annotation", y = "Pearson's r (kinase activity vs kinase phosphosite)") +
  guides(fill = guide_legend(override.aes = list(alpha=1), nrow = 2))

#+ fig.width=2, fig.height=6
KA_RegPsite_cor_distribution

ggsave(filename = "kinase_phosphosite_correlation_psp_status_box2.png", plot = KA_RegPsite_cor_distribution, path = "./output/plots/kinase_reg_psites_correlation/", height = 6, width = 2)
ggsave(filename = "kinase_phosphosite_correlation_psp_status_box2.pdf", plot = KA_RegPsite_cor_distribution, path = "./output/plots/kinase_reg_psites_correlation/", height = 6, width = 2)
unlink("kinase_phosphosite_correlation_psp_status_box2.png")
unlink("kinase_phosphosite_correlation_psp_status_box2.pdf")
...
KA_RegPsite_cor_distribution <- all_cor %>%
  ...
  theme(
    plot.margin = unit(c(0,0,1.5,0), "cm"),
    axis.title.x = element_text(colour = "black", size = 17, hjust = 1.5),
    #axis.title.x = element_text(colour = "black", size = 17, hjust = 1),
    axis.title.y = element_text(colour = "black", size = 17, hjust = 1),
    axis.text.y = element_text(colour = "black", size = 16),
    axis.text.x = element_blank(),
