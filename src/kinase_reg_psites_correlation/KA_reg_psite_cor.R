...
KA_RegPsite_cor_distribution <- all_cor %>%
  ...
  geom_beeswarm(mapping = aes(x = PSP_reg_status, y = pearson_r), alpha = 0.2) +
  scale_fill_manual(values = c("#a6611a", "#018571")) +
  #scale_fill_manual(values = c("#bdbdbd", "#67a9cf")) +
  stat_compare_means(mapping = aes(x = PSP_reg_status, y = pearson_r, label = paste0("p = ", ..p.format..)), method = "wilcox.test", label.y = 1, label.x = 1.2, size = 5) +
  theme_classic() +
  theme(
    axis.title = element_text(colour = "black", size = 18),
    axis.text = element_text(colour = "black", size = 16),
...
ggsave(filename = "kinase_phosphosite_correlation_psp_status_box2.png", plot = KA_RegPsite_cor_distribution, path = "./output/plots/kinase_reg_psites_correlation/", height = 6, width = 3)
ggsave(filename = "kinase_phosphosite_correlation_psp_status_box2.pdf", plot = KA_RegPsite_cor_distribution, path = "./output/plots/kinase_reg_psites_correlation/", height = 6, width = 3)
