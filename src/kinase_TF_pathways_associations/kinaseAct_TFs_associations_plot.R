...
vplot <- associations %>%
  ...
  geom_text_repel(size = 4, segment.size = 0.1, color = "black", direction = "y", hjust = -0.3, seed = 123) +
...
ggsave(filename = "kinase_TFs_associations_volcano_plot2.png", plot = vplot, path = "./output/plots/kinase_TF_pathways_associations/", width = 4, height = 5)
ggsave(filename = "kinase_TFs_associations_volcano_plot2.pdf", plot = vplot, path = "./output/plots/kinase_TF_pathways_associations/", width = 4, height = 5)
...
plot <- associations %>%
  filter(!is.na(weight)) %>%
  mutate(signf = if_else(padj < 0.05 & abs(kin_beta) > 0.5, "2", "1")) %>%
  mutate(signf = fct_relevel(signf, "1")) %>%
  ggplot() +
  ...
  annotate("text", x = 750, y = 0.8, label = str_c("Wilcoxon, p = ", p_value, sep = ""), size = 4) +
  theme_classic() +
  theme(
  ...
  scale_fill_viridis(discrete = T, name = "Associations", labels = c("1" = "Others", "2" = "Significant")) +
  scale_color_viridis(discrete = T, name = "Associations", labels = c("1" = "Others", "2" = "Significant")) +
  scale_alpha_manual(values = c(0.8,0.5), guide = F) +
  labs(x = "String network edge weight", y = "Density")

ggsave(filename = "kinase_TFs_associations_weights_distribution2.png", plot = plot, path = "./output/plots/kinase_TF_pathways_associations/", width = 5, height = 1.5)
ggsave(filename = "kinase_TFs_associations_weights_distribution2.pdf", plot = plot, path = "./output/plots/kinase_TF_pathways_associations/", width = 5, height = 1.5)
...
plot <- associations %>%
  mutate(signf = if_else(padj < 0.05 & abs(kin_beta) > 0.5, "2", "1")) %>%
  ggplot(mapping = aes(x = signf, y = weight, fill = signf)) +
  geom_violin(alpha = 0.8, size = 1, draw_quantiles = c(0.5)) +
  #geom_boxplot(show.legend = F, outlier.shape = NA) +
  geom_jitter(mapping = aes(alpha = signf), width = 0.1, show.legend = F, pch = 21, color = "white") +
  #stat_summary(fun.data=data_summary, show.legend = F) +
  #geom_boxplot(width=0.05, show.legend = F, outlier.shape = NA) +
  #stat_summary(fun=mean, geom="point", size=2, show.legend = F, color = "red") +
  #geom_text(data = N, mapping = aes(x = signf, y = -Inf, label = n), vjust = -1) +
  coord_flip() +
  theme_classic() +
  theme(
  ...
  stat_compare_means(label.y = 700, label.x = 2.4, size = 5, method = "wilcox.test") +
  scale_alpha_manual(values = c(0.05, 0.8)) +
  scale_y_continuous(name = "String network edge weight") +
  #scale_x_discrete(name = "Kinase-TF Associations") +
  guides(fill = guide_legend(override.aes = list(alpha=1, size = 0.5))) +
  scale_fill_manual(values = c("1" = "#fdc086", "2" = "#7fc97f"), labels = c("1" = "Non-significant", "2" = "Significant"), name = "Associations")

ggsave(filename = "kinase_TFs_associations_weights_distribution4.png", plot = plot, path = "./output/plots/kinase_TF_pathways_associations/", width = 5, height = 3)
ggsave(filename = "kinase_TFs_associations_weights_distribution4.pdf", plot = plot, path = "./output/plots/kinase_TF_pathways_associations/", width = 5, height = 3)
...
plot <- examples %>%
  ...
  geom_point(size = 0.8, alpha = 0.2, color = "#cc4c02") +
  geom_smooth(method = "lm", lwd = 1, color = "#2171b5") +
...
ggsave(filename = "kinase_TFs_associations_examples2.png", plot = plot, path = "./output/plots/kinase_TF_pathways_associations/", width = 5, height = 4)
ggsave(filename = "kinase_TFs_associations_examples2.pdf", plot = plot, path = "./output/plots/kinase_TF_pathways_associations/", width = 5, height = 4)
